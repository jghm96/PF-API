...
const executeOrder = async(userId, orderId) => {
  try{
    ...
    if(orderJ.marketOrder){
      ...
      const html = `
                  <div>
                    <h1>Your ${!orderJ.buyOrder ? `${symbolSellJ.symbol}/${symbolBuyJ.symbol}` : `${symbolBuyJ.symbol}/${symbolSellJ.symbol}`} ${orderJ.buyOrder ? 'buy': 'sell'} order for ${!orderJ.buyOrder ? transactionSell.toJSON().withdraw : transactionBuy.toJSON().deposit} ${!orderJ.buyOrder ? symbolSellJ.symbol : symbolBuyJ.symbol} was executed properly</h1>
                    ...
                    <h3>Price of the pair when the order was executed: ${!pairInverso ? (!orderJ.buyOrder ? Number(pair.data.price) : (1/pair.data.price)): (!orderJ.buyOrder ? (1/pair.data.price) : Number(pair.data.price))} </h3>
                  </div>
      `
